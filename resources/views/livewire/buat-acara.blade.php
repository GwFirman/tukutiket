<div>
    <form method="POST" action="{{ route('pembuat.acara.store') }}" enctype="multipart/form-data" class="p-6 space-y-6"
        ...
        @if ($errors->any())
            <div class="bg-red-50
        border-l-4 border-red-500 p-4 mb-6">
...
@if ($currentStep == 1)
    <div>
        <div class="mb-4">
            ...
            <!-- Input tersembunyi -->
            <input type="file" wire:model="banner_acara" name="banner_acara" id="banner_acara" accept="image/*"
                class="hidden" />
        </div>
    ...
    <div>
        ...
        <input type="text" wire:model="nama_acara" name="nama_acara" id="nama_acara"
            class="mt-1 h-10 block w-full rounded-md border border-gray-100 shadow-sm focus:border-sky-500">
    </div>

    <div class="flex gap-5 mt-5">
        <livewire:tanggal-acara wire:key="tanggal-acara" />
        <div class="w-full">
            <label class="mb-1.5 block text-sm font-medium text-gray-700">
                Lokasi
            </label>
            <input type="text" name="lokasi" id="lokasi" name="lokasi" wire:model="lokasi"
    ...
    <div>
        ...
        <textarea name="deskripsi_acara" id="deskripsi" rows="3" wire:model="deskripsi_acara"
    ...
    <div class="w-full">
        ...
        <input type="text" wire:model="info_kontak" name="info_kontak" id="info_kontak" name="info_kontak"
            class=" shadow-theme-xs font-normal focus:border-blue-300 focus:ring-blue-500/10 h-11 w-full  border border-gray-300 rounded-lg bg-transparent px-4 py-2.5 text-lg text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden ">
    </div>
@endif
...
@if ($currentStep == 2)
    <div>
        <div class="mt-5 w-full">
            ...
            <input type="file" wire:model="banner_tiket" name="banner_tiket"
            ...
            <div x-data="{
                ...
                <div class="p-6 rounded-lg border transition-all duration-200"
                    ...
                    <label class="flex cursor-pointer items-center text-sm font-medium text-gray-700 select-none">
                        <input type="radio" name="jenis_tiket" value="gratis" class="sr-only" x-model="selected" />
                ...
                <div class="p-6 rounded-lg border transition-all duration-200"
                    ...
                    <label class="flex cursor-pointer items-center text-sm font-medium text-gray-700 select-none">
                        <input type="radio" name="jenis_tiket" value="berbayar" class="sr-only" x-model="selected" />
                    ...
                    <template class="flex-col gap-4" x-for="(kategori, index) in kategoriList" :key="index">
                        <div class="flex justify-between items-center border rounded-lg p-4 bg-gray-50">
                            <div>
                                ...
                                <p class="text-gray-500 text-sm">Rp <span x-text="kategori.harga"></span></p>
                            </div>
                ...
                <div class="w-full" x-show="selected === 'berbayar'" x-transition>
            ...
            <div class="space-y-6 border-t border-gray-100 p-5 sm:p-6">
                ...
                <div class="flex flex-col w-full justify-center gap-8">
                    <div class="border p-4 rounded-lg flex items-center gap-4" x-data="{ checkboxToggle: false }"
                        ...
                        <input type="number" :disabled="!checkboxToggle" name="maks_pembelian_per_akun"
                            class="
                    ...
                    <div class="border p-4 rounded-lg flex items-center gap-4" x-data="{ ticketLimitToggle: false }"
                        ...
                        <input type="number" :disabled="!ticketLimitToggle" name="maks_tiket_per_transaksi"
...
<div class="flex justify-between">
    @if ($currentStep == 2)
        <button class="bg-gray-600 px-4 py-2 rounded-lg text-white" wire:click="previousStep">Kembali</button>
    @endif
    @if ($currentStep == 2)
        <button @click="kirimKeLivewire()" type="submit" class="bg-green-600 text-white px-4 py-2 rounded-lg hover:bg-green-700 transition">
            Simpan
        </button>
    @endif
    @if ($currentStep == 1)
        <button class="bg-blue-600 px-4 py-2 rounded-lg text-white" wire:click="firstStepSubmit">Lanjut</button>
    @endif
</div>
</form>
</div>
