<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-2">
            <i data-lucide="log-out" class="size-5 text-indigo-600"></i>
            <span class="font-semibold text-gray-800">
                Check-out Tiket - {{ $acara->nama_acara }}
            </span>
        </div>
    </x-slot>
        ...
        <div class="bg-gradient-to-r from-indigo-500 to-purple-600 rounded-2xl p-6 mb-6 text-white">
            <div class="flex items-center justify-between mb-3">
                ...
                <div class="bg-white/20 px-3 py-1 rounded-full backdrop-blur-sm">
                    ...
                    <span class="text-sm font-semibold">Check-out Mode</span>
                </div>
            </div>
            ...
            <div id="scan-result" class="p-6 bg-white border-2 rounded-2xl hidden transition-all h-fit">
                ...
                <div id="scan-details" class="space-y-3 hidden">
                    ...
                    <div id="checkout-time"
                        class="text-center text-sm bg-orange-50 text-orange-700 py-3 px-4 rounded-xl border border-orange-200 hidden">
                        <i data-lucide="clock" class="size-4 inline mr-1"></i>
                        Check-out: <span id="detail-waktu" class="font-semibold"></span>
                    </div>
                </div>
            </div>
        </div>
    ...
    <script>
        ...
        function onScanSuccess(decodedText) {
            ...
            checkoutTime.classList.add("hidden");
            lucide.createIcons();

            fetch("{{ route('pembuat.scan.checkout', $acara->slug) }}", {
                    method: "POST",
                    headers: {
                ...
                .then(res => res.json())
                .then(data => {
                    if (data.status === "valid" && data.tipe === "OUT") {
                        // Success Check-out
                        successSound.play().catch(() => {});

                        box.classList.remove("border-blue-500");
                        box.classList.add("border-orange-500", "animate-pulse");
                        statusBox.className =
                            "p-4 rounded-xl mb-4 text-center font-semibold bg-gradient-to-r from-orange-100 to-amber-100 text-orange-700 border-2 border-orange-300";
                        statusBox.innerHTML =
                            '<div class="flex items-center justify-center gap-2"><i data-lucide="log-out" class="w-6 h-6"></i><span class="text-lg">' +
